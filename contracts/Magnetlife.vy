...
# @notice The Magnet auction house
# @author 
# @license MIT
#
...
@payable
@nonreentrant("lock")
def create_friend_bid(magnet_id: uint256, bid_amount: uint256, sig: Bytes[65]):
    """
    @dev Create a bid.
    ...
    self._create_bid(magnet_id, bid_amount)


@external
@payable
@nonreentrant("lock")
def create_wl_bid(magnet_id: uint256, bid_amount: uint256, sig: Bytes[65]):
    """
    @dev Create a bid.
      Throws if the whitelist is not enabled.
      Throws if the `sig` is invalid.
...
@payable
@nonreentrant("lock")
def create_bid(magnet_id: uint256, bid_amount: uint256):
    """
    @dev Create a bid.
...
def set_owner(_owner: address):
    ...
    assert msg.sender == self.owner, "Caller is not the owner"
    assert _owner != address(0), "Cannot set owner to zero address"
...
def _create_auction():
    ...
    log AuctionCreated(_magnet_id, _start_time, _end_time)
...
def _settle_auction():
    ...
    if self.auction.bidder == address(0):
        self.magnets.transferFrom(self, self.owner, self.auction.magnet_id)
    else:
        self.magnets.transferFrom(
            self, self.auction.bidder, self.auction.magnet_id
        )
        if self.wl_enabled:
    ...
    log AuctionSettled(
        self.auction.magnet_id, self.auction.bidder, self.auction.amount
    )


@internal
@payable
def _create_bid(magnet_id: uint256, amount: uint256):
    if msg.value < amount:
        missing_amount: uint256 = amount - msg.value
        # Try to use the user's pending returns
        assert (
            self.pending_returns[msg.sender] >= missing_amount
        ), "Does not have enough pending returns to cover remainder"
        self.pending_returns[msg.sender] -= missing_amount
    assert self.auction.magnet_id == magnet_id, "Magnet not up for auction"
    ...
    log AuctionBid(self.auction.magnet_id, msg.sender, amount, extended)

    if extended:
        log AuctionExtended(self.auction.magnet_id, self.auction.end_time)
...
def _check_wl_signature(sig: Bytes[65], sender: address) -> bool:
    ...
    ethSignedHash: bytes32 = keccak256(
        concat(
            ...
            keccak256(abi.encode("whitelist:", sender)),
        )
    )
...
def _check_friend_signature(sig: Bytes[65], sender: address) -> bool:
    ...
    ethSignedHash: bytes32 = keccak256(
        concat(
            b"\x19Ethereum Signed Message:\n32",
            keccak256(abi.encode("friend:", sender)),
        )
    )
